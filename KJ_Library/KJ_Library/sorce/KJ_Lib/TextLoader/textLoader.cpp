...
bool	textLoader::Open( LPCSTR filename )
{
	FileBuf = NULL;
	//	ファイル読み込み
	HANDLE	hfile;
	...
	if( hfile == INVALID_HANDLE_VALUE ){
		MessageBox(NULL,TEXT(filename),TEXT("わーにん！ファイル読み込み失敗"),MB_OK|MB_ICONWARNING);
		return false;
	}
...
textLoader::textLoader( LPCSTR filename )
{
	bInit = Open(filename);
}
...
//	検索
//------------------------------------------------------
bool	textLoader::Search( LPCSTR str )
{
	char	temp[128];
...
//	行内検索
//------------------------------------------------------
bool	textLoader::SearchLine( LPCSTR str )
{
	char	temp[128];
...
//	階層内検索
//------------------------------------------------------
bool	textLoader::Search2( LPCSTR str )
{
	char	temp[128];
	int		work = 0;
...
//	同一インデント内検索
//------------------------------------------------------
bool	textLoader::SearchIndent( LPCSTR str )
{
	char	temp[128];
	int		work = 0;
